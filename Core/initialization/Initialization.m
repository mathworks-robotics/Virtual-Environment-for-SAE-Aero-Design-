...
%% Initial Position
% Position is selected by the Environment mask. Presets are read from the
% environment data dictionary; custom definitions use the mask fields.

environmentBlocks = find_system(bdroot, ...
    'SearchDepth', 1, ...
    'MatchFilter', @Simulink.match.allVariants, ...
    'BlockType', 'SubSystem', ...
    'MaskType', 'SAE Aero Environment');
if numel(environmentBlocks) ~= 1
    error('Initialization:EnvironmentBlock', ...
        'Expected one top-level SAE Aero Environment subsystem.');
end
activeEnvironment = EnvironmentMask.resolveForBlock(environmentBlocks{1});

initial.Latitude = activeEnvironment.Location.Latitude_deg;        % [deg]
initial.Longitude = activeEnvironment.Location.Longitude_deg;      % [deg]
initial.Altitude_MSL = activeEnvironment.Location.Elevation_MSL_m; % [m]
