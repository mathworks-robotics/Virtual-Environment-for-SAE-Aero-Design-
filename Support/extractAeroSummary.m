...
%                .rudderData
%                .aileronData
%              On return, summary and interpolation-limit fields are added:
%                .CLmax          [-]
%                .CD0            [-]
%                .CLalpha_perDeg [1/deg]
%                .validAlphaRange_deg [1x2 deg]
%                .validBetaRange_deg  [1x2 deg]
    ...
    betaVec     = aeroData.rudderData.Beta;                   % [nBeta x 1]

    % Saturation limits used by the aerodynamic protection blocks.
    aeroData.validAlphaRange_deg = [min(alphaVec(:)), max(alphaVec(:))];
    aeroData.validBetaRange_deg  = [min(betaVec(:)), max(betaVec(:))];
