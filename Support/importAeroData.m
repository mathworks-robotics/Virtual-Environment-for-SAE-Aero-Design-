...
function data = importElevAilSheet(filename, sheetName)
    ...
    for i = 1:height(rawData)
        iCtrl  = find(controlDeflections == rawData.(controlColName)(i));
        iAlpha = find(alphaValues        == rawData.alpha(i));
...
function data = importRudderSheet(filename)
    ...
    for i = 1:height(rawData)
        iCtrl = find(controlDeflections == rawData.(controlColName)(i));
        iBeta = find(betaValues         == rawData.Beta(i));
        for j = 1:length(paramNames)
            p = paramNames{j};
...
% Row 5 = headers, Row 6+ = data. Non-numeric Value rows are skipped.
    ...
    opts = detectImportOptions(filename, 'Sheet', 'StabilityDerivatives');
    opts.VariableNamesRange = 'A5';
    opts.DataRange          = 'A6';
    opts.VariableNamingRule = 'preserve';
    raw = readtable(filename, opts);

    % Drop blank / section-header rows (Value = NaN)
    raw = raw(~isnan(raw.Value), :);
...
function plotAeroData(data, parameter, varargin)
...
% Fetch one derivative by name; error clearly if missing.
    if derivMap.isKey(name)
        val = derivMap(name);
    else
        error('importAeroData:missingDerivative', ...
            ['Derivative "%s" not found in StabilityDerivatives sheet.\n' ...
             'Check column A — names are case-sensitive and must match exactly.'], name);
    end
end
